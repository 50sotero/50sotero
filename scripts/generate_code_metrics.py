...
def _render_header(card: MetricsCard) -> str:
    return f"""  <rect x="16" y="16" width="888" height="608" rx="22" fill="url(#card)" stroke="#303c4d" filter="url(#shadow)"/>
...
def _render_stats(card: MetricsCard, net_sign: str, net_lines: int) -> str:
...
def _render_commit_chart(latest: MonthMetric, area: str, line: str, max_val: float, ticks: str) -> str:
    return f"""  <g>
    ...
    <path d="{line}" stroke="#58a6ff" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
    <text x="74" y="397" class="axis">0</text><text x="414" y="397" class="axis" text-anchor="end">peak {max_val:g}</text>
    {ticks}
  </g>"""


def _render_changed_chart(latest: MonthMetric, area: str, line: str, max_val: float, ticks: str) -> str:
    return f"""  <g>
    ...
    <path d="{line}" stroke="#f97316" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
    <text x="506" y="397" class="axis">0</text><text x="846" y="397" class="axis" text-anchor="end">peak {format_compact(max_val)}</text>
    {ticks}
  </g>"""


def _render_languages(best_text: str, language_bar: str, language_legend: str) -> str:
    return f"""  <text x="54" y="472" class="label">SOURCE LOC MIX</text>
...
def render_svg(card: MetricsCard) -> str:
    """Render the profile metrics SVG."""
...
{_render_stats(card, net_sign, net_lines)}

{_render_commit_chart(latest, commit_area, commit_line, commit_max, commit_ticks)}

{_render_changed_chart(latest, changed_area, changed_line, changed_max, changed_ticks)}

{_render_languages(best_text, language_bar, language_legend)}
</svg>
"""
